...
def check_sampling_frequency_and_duration(input_folder, extension='ogg'):
    ...
    durations = list()
    for wave_file in glob.glob(search_string, recursive=True):
        print("Checking Fs and duration of {}...".format(wave_file))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--label_file', help='label file', default='../train_metadata.csv')
    # default is false
    ...
    durations = check_sampling_frequency_and_duration(
        args.input_folder, extension='ogg')
